...
if (isset($_POST['search']) && isset($_POST['filter'])) {
    try {
        ...
        $kind_method = $_POST['filter'];
        
        $sql_search = 'SELECT * FROM request_job WHERE title LIKE :src_val AND kind_request = :kind_method ORDER BY date_now DESC';
        $stmt_search = $conn->prepare($sql_search);
        $search_value = '%' . $search_value . '%';
        $stmt_search->bindParam(':src_val', $search_value);
        $stmt_search->bindParam(':kind_method', $kind_method);
        $stmt_search->execute();
